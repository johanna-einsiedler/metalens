#!/usr/bin/env python3
"""Regenerate paperlens/static/models.json — the model list + pricing the extract page
loads at startup (Metalens and MASEMiner).
...
Usage:
    python update_models.py [path/to/models.json]
        (defaults to paperlens/static/models.json relative to repo root)
"""
...
def main() -> int:
    ...
    out_path = Path(sys.argv[1]) if len(sys.argv) > 1 else repo_root / "paperlens" / "static" / "models.json"
